## boot_d1.tcl - unattended boot of the co-gen D2mux image (csi_mux inserted).
## Mirrors boot_inline.tcl but loads BOOT_d2mux.BIN + the ZOCL dtb (D1 is XRT) +
## rootfs_d2mux (d2mux-validate autorun). Board host via env BOARD_URL (e.g. TCP:morelNN:3121).
...
puts stderr "INFO: device program BOOT_d2mux.BIN"
...
puts stderr "INFO: rootfs_d2mux (239M, several min) -> 0x04000000"; dow -data $IMG/rootfs_d2mux.cpio.gz.u-boot 0x04000000
after 500; catch {con}
puts stderr "INFO: BOOT_RELEASED - autoboot should bring up Linux + d2mux-validate autorun"
exit 0

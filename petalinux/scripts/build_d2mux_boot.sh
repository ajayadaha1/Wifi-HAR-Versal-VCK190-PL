#!/bin/bash
# build_d2mux_boot.sh - BOOT_d2mux.BIN for the co-gen D1 XSA (csi_mux inserted).
# Mirrors build_inline_boot.sh but: D1 PDI (vpl_gen_fixed.pdi from inline_cogen_d2mux.xsa),
# D1 co-gen aie CDO (package_d2mux/aie.merged.cdo.bin), and the ZOCL dtb
...
WORK=$ROOT/petalinux/d2mux_boot
XSA=$ROOT/aie/feature_graph/inline_cogen_d2mux.xsa
AIE_CDO=$ROOT/aie/feature_graph/package_d2mux/aie.merged.cdo.bin
...
mv -f "$WORK/vpl_gen_fixed.pdi" "$WORK/d2mux.pdi"
ls -l "$WORK/d2mux.pdi"
cat > "$WORK/bootgen_d2mux.bif" <<EOF
the_ROM_image:
{
image {
        { type=bootimage, file=$WORK/d2mux.pdi }
...
EOF
echo "== bootgen"
bootgen -arch versal -image "$WORK/bootgen_d2mux.bif" -w -o "$IMG/BOOT_d2mux.BIN"
ls -l "$IMG/BOOT_d2mux.BIN"
echo "BOOT_D2MUX_DONE"

// Fix variant: the packaged AIE CDO enables (free-runs) the graph at boot, so
// xrt::graph::wait() never returns a run-count completion. We therefore do NOT
// wait on the graph — we feed mm2s and drain s2mm, and rely on s2mm.wait()
// (completes when the 3 result words actually arrive). Keeps the [diag] prints.
#include <cmath>
#include <cstdio>
#include <fstream>
#include <vector>
...
int main(int argc, char** argv) {
    ...
    // NOTE: NO graph.wait() — the PDI enables/free-runs the graph so the
    // run-count completion never fires; s2mm.wait() (data-driven) is the real barrier.
    STEP("WAIT s2mm ...");
    r_s2mm.wait();
    STEP("s2mm DONE");
